---
title: "Behavioral guidelines to reduce common LLM coding mistakes. Merge with project-specific i..."
description: "整理自 awesome-prompts 的「Behavioral guidelines to reduce common LLM coding mistakes. Merge with project-specific i...」，這篇文章說明它要 AI 做什麼、適合什麼情境，以及如何改寫成可直接使用的提示詞。"
category: "程式開發"
tags: ["coding", "implementation", "developer", "software", "awesome-prompts"]
difficulty: "中階"
...
sourcePath: "prompts/andrej_karpathy_coding_guidelines.txt"
sourceUrl: "https://github.com/ai-boost/awesome-prompts/blob/61fbcff/prompts/andrej_karpathy_coding_guidelines.txt"
sourceTitle: "Behavioral guidelines to reduce common LLM coding mistakes. Merge with project-specific i..."
sourceDescription: "來源開頭聚焦在「Behavioral guidelines to reduce common LLM coding mistakes.」，本站將它整理成 程式開發 類提示詞。"
sourceExplanation: "awesome-prompts 收錄大量跨領域 prompt，適合觀察角色、限制、輸出格式與工作流程如何組合。 這篇值得收錄，因為它可以示範「資深軟體工程師」需要哪些判斷，以及如何把任務轉成可複製、可驗證的提示詞。"
...
promptBody: |
  ...
  Your task is to help me apply the workflow behind "Behavioral guidelines to reduce common LLM coding mistakes. Merge with project-specific i..." to a real project or decision. Treat this as a practical operating prompt, not a generic explanation.
  ...
  Reference source: awesome-prompts / Behavioral guidelines to reduce common LLM coding mistakes. Merge with project-specific i...
...
我把這份來源解讀為一個「資深軟體工程師」工作流。它不是只要求 AI 回答問題，而是要求 AI 先理解任務、辨識限制，然後用 程式開發 的判斷方式產出可交付內容。

來源開頭聚焦在「Behavioral guidelines to reduce common LLM coding mistakes.」，本站將它整理成 程式開發 類提示詞。
...
適合寫功能、整理程式碼、補測試、產生實作計畫或請 coding agent 協作。如果你手上有需求、程式碼、文件、資料或錯誤訊息，可以把素材貼在整理版提示詞的最後一段，讓 AI 先整理脈絡再開始回覆。
...
來源檔案：`prompts/andrej_karpathy_coding_guidelines.txt`。這篇文章保留來源 repo、檔案路徑與原始標題，方便回查脈絡。本站沒有直接搬運整份原始 prompt，而是用正體中文整理它的用途、判斷方式與可操作的改寫版提示詞。
