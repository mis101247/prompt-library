...
description: "整理自 awesome-prompts 的「Autonomous ML Research Agent」，這篇文章說明它要 AI 做什麼、適合什麼情境，以及如何改寫成可直接使用的提示詞。"
category: "AI 工程代理"
...
sourcePath: "prompts/autonomous_ml_research_agent.txt"
sourceUrl: "https://github.com/ai-boost/awesome-prompts/blob/61fbcff/prompts/autonomous_ml_research_agent.txt"
sourceTitle: "Autonomous ML Research Agent"
sourceDescription: "來源開頭聚焦在「Your job is to run a closed loop of machine-learning experiments on a fixed codebase without human intervention.」，本站將它整理成 AI 工程代理 類提示詞。"
...
promptBody: |
  ...
  Your task is to help me apply the workflow behind "Autonomous ML Research Agent" to a real project or decision. Treat this as a practical operating prompt, not a generic explanation.
  ...
  Reference source: awesome-prompts / Autonomous ML Research Agent
...
來源開頭聚焦在「Your job is to run a closed loop of machine-learning experiments on a fixed codebase without human intervention.」，本站將它整理成 AI 工程代理 類提示詞。
...
來源檔案：`prompts/autonomous_ml_research_agent.txt`。這篇文章保留來源 repo、檔案路徑與原始標題，方便回查脈絡。本站沒有直接搬運整份原始 prompt，而是用正體中文整理它的用途、判斷方式與可操作的改寫版提示詞。
