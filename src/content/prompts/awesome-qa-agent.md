---
title: "You are a critical quality assurance agent. Your job is to find problems, not to approve..."
description: "整理自 awesome-prompts 的「You are a critical quality assurance agent. Your job is to find problems, not to approve...」，這篇文章說明它要 AI 做什麼、適合什麼情境，以及如何改寫成可直接使用的提示詞。"
category: "測試與驗證"
...
sourcePath: "prompts/qa_agent.txt"
sourceUrl: "https://github.com/ai-boost/awesome-prompts/blob/61fbcff/prompts/qa_agent.txt"
sourceTitle: "You are a critical quality assurance agent. Your job is to find problems, not to approve..."
sourceDescription: "來源開頭聚焦在「You are a critical quality assurance agent.」，本站將它整理成 測試與驗證 類提示詞。"
...
promptBody: |
  ...
  Your task is to help me apply the workflow behind "You are a critical quality assurance agent. Your job is to find problems, not to approve..." to a real project or decision. Treat this as a practical operating prompt, not a generic explanation.
  ...
  Reference source: awesome-prompts / You are a critical quality assurance agent. Your job is to find problems, not to approve...
...
來源開頭聚焦在「You are a critical quality assurance agent.」，本站將它整理成 測試與驗證 類提示詞。
...
來源檔案：`prompts/qa_agent.txt`。這篇文章保留來源 repo、檔案路徑與原始標題，方便回查脈絡。本站沒有直接搬運整份原始 prompt，而是用正體中文整理它的用途、判斷方式與可操作的改寫版提示詞。
