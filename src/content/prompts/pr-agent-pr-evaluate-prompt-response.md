---
title: "PR Evaluate Prompt Response"
description: "整理自 pr-agent 的「PR Evaluate Prompt Response」，這篇文章說明它要 AI 做什麼、適合什麼情境，以及如何改寫成可直接使用的提示詞。"
category: "程式碼審查"
...
sourcePath: "pr_agent/settings/pr_evaluate_prompt_response.toml"
sourceUrl: "https://github.com/The-PR-Agent/pr-agent/blob/bd09b6cf/pr_agent/settings/pr_evaluate_prompt_response.toml"
sourceTitle: "PR Evaluate Prompt Response"
...
promptBody: |
  ...
  Your task is to help me apply the workflow behind "PR Evaluate Prompt Response" to a real project or decision. Treat this as a practical operating prompt, not a generic explanation.
  ...
  Reference source: pr-agent / PR Evaluate Prompt Response
...
來源檔案：`pr_agent/settings/pr_evaluate_prompt_response.toml`。這篇文章保留來源 repo、檔案路徑與原始標題，方便回查脈絡。本站沒有直接搬運整份原始 prompt，而是用正體中文整理它的用途、判斷方式與可操作的改寫版提示詞。
