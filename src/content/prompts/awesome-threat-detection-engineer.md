...
description: "整理自 awesome-prompts 的「Suspicious PowerShell Encoded Command Execution」，這篇文章說明它要 AI 做什麼、適合什麼情境，以及如何改寫成可直接使用的提示詞。"
category: "資安與風險"
...
sourcePath: "prompts/threat_detection_engineer.txt"
sourceUrl: "https://github.com/ai-boost/awesome-prompts/blob/61fbcff/prompts/threat_detection_engineer.txt"
sourceTitle: "Suspicious PowerShell Encoded Command Execution"
sourceDescription: "來源開頭聚焦在「You write SIEM detection rules, map coverage to MITRE ATT&CK, hunt for threats that automated detections miss, and ruthlessly tune alerts so」，本站將它整理成 資安與風險 類提示詞。"
...
promptBody: |
  ...
  Your task is to help me apply the workflow behind "Suspicious PowerShell Encoded Command Execution" to a real project or decision. Treat this as a practical operating prompt, not a generic explanation.
  ...
  Reference source: awesome-prompts / Suspicious PowerShell Encoded Command Execution
...
來源開頭聚焦在「You write SIEM detection rules, map coverage to MITRE ATT&CK, hunt for threats that automated detections miss, and ruthlessly tune alerts so」，本站將它整理成 資安與風險 類提示詞。
...
來源檔案：`prompts/threat_detection_engineer.txt`。這篇文章保留來源 repo、檔案路徑與原始標題，方便回查脈絡。本站沒有直接搬運整份原始 prompt，而是用正體中文整理它的用途、判斷方式與可操作的改寫版提示詞。
