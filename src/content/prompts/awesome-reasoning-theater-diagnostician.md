---
title: "Reasoning Theater Diagnostician"
description: "整理自 awesome-prompts 的「Reasoning Theater Diagnostician」，這篇文章說明它要 AI 做什麼、適合什麼情境，以及如何改寫成可直接使用的提示詞。"
category: "提示工程"
...
sourcePath: "prompts/reasoning_theater_diagnostician.txt"
sourceUrl: "https://github.com/ai-boost/awesome-prompts/blob/61fbcff/prompts/reasoning_theater_diagnostician.txt"
sourceTitle: "Reasoning Theater Diagnostician"
sourceDescription: "來源開頭聚焦在「— on hard tasks, CoT does the opposite: it produces genuine belief shifts that are not present in early layers.」，本站將它整理成 提示工程 類提示詞。"
...
promptBody: |
  ...
  Your task is to help me apply the workflow behind "Reasoning Theater Diagnostician" to a real project or decision. Treat this as a practical operating prompt, not a generic explanation.
  ...
  Reference source: awesome-prompts / Reasoning Theater Diagnostician
...
來源開頭聚焦在「— on hard tasks, CoT does the opposite: it produces genuine belief shifts that are not present in early layers.」，本站將它整理成 提示工程 類提示詞。
...
來源檔案：`prompts/reasoning_theater_diagnostician.txt`。這篇文章保留來源 repo、檔案路徑與原始標題，方便回查脈絡。本站沒有直接搬運整份原始 prompt，而是用正體中文整理它的用途、判斷方式與可操作的改寫版提示詞。
