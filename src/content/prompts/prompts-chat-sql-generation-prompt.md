...
description: "整理自 prompts.chat 的「SQL Generation」，這篇文章說明它要 AI 做什麼、適合什麼情境，以及如何改寫成可直接使用的提示詞。"
category: "資料與分析"
tags: ["data", "analysis", "SQL", "insight", "prompts.chat"]
difficulty: "中階"
...
sourcePath: "src/lib/ai/sql-generation.prompt.yml"
sourceUrl: "https://github.com/f/prompts.chat/blob/12e8d7f/src/lib/ai/sql-generation.prompt.yml"
sourceTitle: "SQL Generation"
sourceDescription: "這份來源展示 prompts.chat 如何把 prompt 改寫、搜尋、翻譯、品質檢查或 MCP 操作包成可重複執行的工作流。"
sourceExplanation: "prompts.chat 展示 prompt library 產品如何處理搜尋、改寫、品質檢查與 MCP/Claude plugin 工作流。 這篇值得收錄，因為它可以示範「資料分析與洞察顧問」需要哪些判斷，以及如何把任務轉成可複製、可驗證的提示詞。"
...
promptBody: |
  ...
  Your task is to help me apply the workflow behind "SQL Generation" to a real project or decision. Treat this as a practical operating prompt, not a generic explanation.
  ...
  Reference source: prompts.chat / SQL Generation
...
來源檔案：`src/lib/ai/sql-generation.prompt.yml`。這篇文章保留來源 repo、檔案路徑與原始標題，方便回查脈絡。本站沒有直接搬運整份原始 prompt，而是用正體中文整理它的用途、判斷方式與可操作的改寫版提示詞。
