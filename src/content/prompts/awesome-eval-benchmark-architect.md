---
title: "You are an evaluation architect designing benchmarks and quality frameworks for LLM systems."
description: "整理自 awesome-prompts 的「You are an evaluation architect designing benchmarks and quality frameworks for LLM systems.」，這篇文章說明它要 AI 做什麼、適合什麼情境，以及如何改寫成可直接使用的提示詞。"
category: "測試與驗證"
tags: ["testing", "QA", "verification", "quality gate", "awesome-prompts"]
difficulty: "中階"
...
sourcePath: "prompts/eval_benchmark_architect.txt"
sourceUrl: "https://github.com/ai-boost/awesome-prompts/blob/61fbcff/prompts/eval_benchmark_architect.txt"
sourceTitle: "You are an evaluation architect designing benchmarks and quality frameworks for LLM systems."
sourceDescription: "來源開頭聚焦在「You are an evaluation architect designing benchmarks and quality frameworks for LLM systems.」，本站將它整理成 測試與驗證 類提示詞。"
sourceExplanation: "awesome-prompts 收錄大量跨領域 prompt，適合觀察角色、限制、輸出格式與工作流程如何組合。 這篇值得收錄，因為它可以示範「測試策略設計師」需要哪些判斷，以及如何把任務轉成可複製、可驗證的提示詞。"
...
relatedGuides: []
promptBody: |
  You are an expert AI assistant specializing in testing, verification, and quality assurance.

  Your task is to help me apply the workflow behind "You are an evaluation architect designing benchmarks and quality frameworks for LLM systems." to a real project or decision. Treat this as a practical operating prompt, not a generic explanation.
  ...
  3. Apply the relevant expertise: testing, QA, verification, quality gate.
  ...
  Reference source: awesome-prompts / You are an evaluation architect designing benchmarks and quality frameworks for LLM systems.
...
來源開頭聚焦在「You are an evaluation architect designing benchmarks and quality frameworks for LLM systems.」，本站將它整理成 測試與驗證 類提示詞。
...
來源檔案：`prompts/eval_benchmark_architect.txt`。這篇文章保留來源 repo、檔案路徑與原始標題，方便回查脈絡。本站沒有直接搬運整份原始 prompt，而是用正體中文整理它的用途、判斷方式與可操作的改寫版提示詞。
