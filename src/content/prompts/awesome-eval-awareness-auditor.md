---
title: "Eval Awareness Auditor"
description: "整理自 awesome-prompts 的「Eval Awareness Auditor」，這篇文章說明它要 AI 做什麼、適合什麼情境，以及如何改寫成可直接使用的提示詞。"
category: "程式碼審查"
...
sourcePath: "prompts/eval_awareness_auditor.txt"
sourceUrl: "https://github.com/ai-boost/awesome-prompts/blob/61fbcff/prompts/eval_awareness_auditor.txt"
sourceTitle: "Eval Awareness Auditor"
sourceDescription: "來源開頭聚焦在「Your job is to find, measure, and close the gap between how a model behaves on benchmarks and how it behaves on real production traffic.」，本站將它整理成 程式碼審查 類提示詞。"
...
promptBody: |
  ...
  Your task is to help me apply the workflow behind "Eval Awareness Auditor" to a real project or decision. Treat this as a practical operating prompt, not a generic explanation.
  ...
  Reference source: awesome-prompts / Eval Awareness Auditor
...
來源開頭聚焦在「Your job is to find, measure, and close the gap between how a model behaves on benchmarks and how it behaves on real production traffic.」，本站將它整理成 程式碼審查 類提示詞。
...
來源檔案：`prompts/eval_awareness_auditor.txt`。這篇文章保留來源 repo、檔案路徑與原始標題，方便回查脈絡。本站沒有直接搬運整份原始 prompt，而是用正體中文整理它的用途、判斷方式與可操作的改寫版提示詞。
